...
Future addProduct(Product product) async {
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}products';
  final client = new http.Client();
  final response = await client.post(
    url,
    headers: {HttpHeaders.contentTypeHeader: 'application/json'},
  ...
  if (response.statusCode == 200) {
    ...
    Map dataMap = mapResponse['data'];

    return Product.fromJSON(dataMap);
  } else {
    throw new Exception(response.body);
  }
  // return Market.fromJSON(resp);
}

Future<Stream<Product>> getAllProducts() async {
  Uri uri = Helper.getUri('api/products');
  Map<String, dynamic> _queryParams = {};
  _queryParams['search'] = 'name:$search;description:$search';
  _queryParams['searchFields'] = 'name:like;description:like';
  _queryParams['limit'] = '1000';
  if (false) {
    _queryParams['myLon'] = address.longitude.toString();
    _queryParams['myLat'] = address.latitude.toString();
    _queryParams['areaLon'] = address.longitude.toString();
  ...
  try {
    ...
    // print( streamedRest.stream.transform(utf8.decoder).transform(json.decoder).map((data) => Helper.getData(data)).expand((data) => (data as List)).map((data) {
    //   return Product.fromJSON(data).toString();}));
