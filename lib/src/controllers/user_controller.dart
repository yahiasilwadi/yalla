        ...
        if (value != null && value.apiToken != null) {
          if (repository.currentUser.value.phone == null) {
            scaffoldKey?.currentState?.showSnackBar(SnackBar(
              content: Text('الرجاء اكمال بياناتك الشخصية'),
            ));

            Navigator.of(context).pushReplacementNamed('/Settings');
          } else {
            print(repository.currentUser.value.id);
  ...
  void register() async {
    ...
    if (loginFormKey.currentState.validate()) {
      ...
      repository.register(user).then((value) {
        if (value != null && value.apiToken != null) {
          //   Navigator.of(scaffoldKey.currentContext).pushReplacementNamed('/Settings',arguments:Text('الرجاء اكمال بياناتك الشخصية') );

          /*Navigator.of(scaffoldKey.currentContext)
              .pushReplacementNamed('/Settings',
                  arguments: RouteArgument(
                    param: {
                      'text': Text('الرجاء إكمال ملفك الشخصي'),
                      'key': scaffoldKey
                    },
                  ));*/
          Navigator.of(scaffoldKey.currentContext)
